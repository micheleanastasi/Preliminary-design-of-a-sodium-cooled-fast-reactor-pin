...
zz = np.linspace(pin_bottom_pos, pin_top_pos, 20)
    ...
    for j in range(len(rr)-1, -1, -1): # da len-1 to 0 index
        ...
        if r_c == 0 and r_v == 0: # zones where there isn't restr.
            ...
            if rr[j] >= r_c: # as fabricated
                ...
                temp_matrix[i,j] = temp_fuel_inner_radial(rr[j], zz[i], clad_d_out_hot, fuel_d_out_hot, clad_thickness_0,x=0,pu=0.29,po=0.05,r_c=r_c,temp_clmn=last_one)
            elif rr[j] <= r_v: # void region
                temp_matrix[i,j] = temp_fuel_inner_radial(r_v, zz[i], clad_d_out_hot, fuel_d_out_hot, clad_thickness_0,x=0,pu=0.29,po=0.05,r_c=r_c,temp_clmn=last_one)
...
plt.figure(2)
plt.plot(rr,temp_matrix[10,:])
plt.plot(rr,temp_before[10,:])
