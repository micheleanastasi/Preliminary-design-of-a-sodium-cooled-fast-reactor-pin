...
for i in range(0, len(zz)): # Z axis
    _, zz_temp[i, :], _, _, clad_d_out_hot, fuel_d_out_hot = hot_geometry_general(zz[i],clad_d_outer,fuel_d_outer,clad_thickness_0,print_status=False)
    diam, old_diam, zz_r_clmn[i], zz_r_void[i], t_void, t_old = fuel_restructuring(zz[i], zz_temp[i, 3], zz_temp[i, 4], fuel_d_out_hot,clad_d_out_hot)
